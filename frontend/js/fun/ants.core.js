(function () {
  ...
  class Apple {
    // Legt einen Apfel an einer Position an (mit leichter Wackel-Phase).
    constructor(x, y) {
    ...
    // Zeichnet den Apfel (mit sanftem Auf-/Ab-Wippen), sofern nicht eingesammelt.
    draw(ctx, t) {
  ...
  // Eine einzelne Ameise: folgt dem Pfad, sammelt Aepfel und weicht Barrieren aus.
  class Ant {
    constructor(path, idx = 0) {
      ...
      this.apple    = null;  // Apfel-Referenz; nach dem Aufnehmen fuer die Anzeige behalten
      this.done     = false;
    }

    // Lenkkraft in Richtung eines Zielpunkts (begrenzt auf MAX_FORCE).
    steer(target) {
      return target.sub(this.pos).norm().scale(this.speed).sub(this.vel).limit(MAX_FORCE);
    }

    // Fluchtkraft weg von einer Bedrohung innerhalb des Radius r.
    flee(threat, r) {
    ...
    // Naechstgelegenen Pfadindex im lokalen Fenster um die aktuelle Position finden.
    closestIdx() {
    ...
    // Ein Simulationsschritt: Zustand, Lenk-/Trenn-/Fluchtkraefte und Bewegung.
    ...
    // Zeichnet die Ameise (Beine, Koerper, Fuehler und ggf. getragenen Apfel).
    draw(ctx) {
      ctx.save();
  ...
  class Barrier {
    // Legt eine Barriere an der Position an (schwache Anfangsstaerke).
    constructor(x, y) {
      this.pos      = new Vec2(x, y);
      this.strength = 0.2;
      this.alive    = true;
    }
    // Folgt der Maus und verstaerkt sich; ohne Maus schwaecht sie ab und stirbt.
    update(mouse) {
  ...
  class AntSim {
    // Baut Haupt- und Overlay-Canvas, initialisiert Zustand, Aepfel und Events.
    constructor() {
    ...
    // Passt Canvas-Groesse an Fenster/Dokument an und generiert den Pfad neu.
    resize() {
      const W = window.innerWidth;
      const H = docHeight();
      this.canvas.width  = W;
    ...
    // Leitet eine pfadnahe Ameise auf die Overlay-Ebene um (laeuft ueber den Inhalt).
    ...
    // Findet den naechstgelegenen Pfadindex zu pos (optional bevorzugt rechts).
    ...
    // Fuehrt eine Overlay-Ameise zurueck als normale Pfad-Ameise.
    ...
    // Zeichnet eine Overlay-Ameise (eigene Darstellung ueber dem Inhalt).

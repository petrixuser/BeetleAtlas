...
## Stand 2026-06-17 (Ist-Zustand)

Erledigt:
- Auth Register/Login/Refresh/Logout/me mit JWT Access + Refresh Rotation.
- RBAC zentral (researcher/admin/viewer Rollenmodell), Write-Endpunkte abgesichert.
- Owner-Update-Regel: Creator darf eigenen Datensatz patchen, Fremd-Datensatz nicht.
- Soft Delete für beetle_records.
- Einheitliches Error-Envelope.
- Login/Register/Refresh/Bootstrap Rate-Limits.
- Redis-Fallback-Strategie: Redis zuerst, In-Memory fallback.
- Redis in Dev und Prod Compose verdrahtet.
- Admin-Bootstrap über /auth/bootstrap-admin (env-gated + token-gated).
- SQL-Seed für initialen Admin vorhanden.
- Audit-Log-Tabelle für beetle create/update/delete vorhanden.
- CamelCase Response-Felder für Audit-/Zeitfelder (createdAt, updatedAt, ...).
- Contract-Tests für researcher/admin plus viewer write-denied ergänzt.

Hinweis zu "researcher auswählen" und "generelles Admin-Passwort":
- Researcher-Auswahl beim Account-Create ist bereits möglich über role="researcher" im Register-Request.
- Admin über öffentliches "generelles Admin-Passwort" beim normalen Login ist bewusst NICHT implementiert (unsicher).
- Stattdessen sicherer Ansatz: Admin-Create nur über Bootstrap-Endpoint mit separatem Bootstrap-Token oder SQL-Seed.

Runbook: Abschluss in der Zielumgebung
...
3) Backend + Redis starten
- Dev: docker-compose.dev.yml
- Prod: docker-compose.prod.yml

4) Contract-Tests ausführen
- backend/tests/test_api_contract.py
- backend/tests/test_api_error_contract.py
- backend/tests/test_api_rbac_contract.py

Finale Ready-Checklist

- [ ] schema_migrations enthält:
	- 20260617_01_auth_manual_beetles
	- 20260617_02_auth_refresh_tokens
	- 20260617_03_beetle_record_audit
- [ ] app_user, app_refresh_token, beetle_record, beetle_record_audit existieren.
- [ ] Mindestens ein Admin vorhanden (Bootstrap oder SQL-Seed).
- [ ] Refresh-Flow getestet: login -> refresh -> logout/revoke.
- [ ] RBAC getestet: researcher create ok, foreign update 403, delete 403; admin update/delete ok; viewer write 403.
- [ ] Redis erreichbar in Laufzeitumgebung (oder bewusst Fallback akzeptiert).
- [ ] Alle 3 Contract-Testdateien laufen gruen.

Optionales Aufraeumen (nicht blockierend)
- Alte blueprint/*_blueprint.py Dateien mit NotImplementedError entfernen oder archivieren.

## Zu erledigen:

1. Es gibt ja noch auf der Karte die Option Länder anzuklicken. Gib den Ländern bitte Informationen zu Höhendaten, Vegetationen, top 3 Käfer usw. Welche Informationen könnten noch interessant sein? Kann man die Infos aus dem Backend ziehen in dem man die Koordinaten anschaut und den Ländern zuweist oder gibt es da eine bessere Option.

2. Bei den filtern mb mehr noch zir auswahl geben? Und wenn man auf so nem filter ist wie z.b vegetation siehst du ja links solegende, mb auch dafür dann filter noch machen? Also dass man dann auf tundra klickt und dann ist da nur tundra ausgewählt so yk NUR FRONTENDKARTENFILTER AUS DER GOOGLE API MÖGLICH?

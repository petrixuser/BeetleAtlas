...
- API-Endpunkte gemaess Zielvertrag: `/api/beetles/:id`, `/api/map/points`, `/api/countries/:countryCode`
...
- `/api/beetles/:id`
- `/api/map/points` (inkl. spaeterer Cluster-/Aggregationslogik)
- `/api/countries/:countryCode`
- final einheitliches Response-Format fuer alle Listenendpunkte (z. B. `items`, `total`, `page`)
...
- Priorisierung und Implementierungsreihenfolge der noch offenen API-Endpunkte
  (`/api/map/points`, `/api/beetles/:id`, `/api/countries/:countryCode`).

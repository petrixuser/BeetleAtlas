...
2026-06-05 (Delta Backend/DB):

  Ordner angepasst.
  Bitte kurz pruefen, ob lokale Datei-Pfade/Directories bei dir noch stimmen.

  Backend heute zusammengefuehrt (Kurzstand):
  - Backend liegt unter `Database/backend/` (FastAPI + MySQL-Anbindung).
  - Laufende Basis-Endpunkte: `/health`, `/stats/overview`, `/species`, `/observations`,
    `/climate/location/{location_id}`.
  - Frontend-nahe Endpunkte: `/api/beetles`, `/api/filters`, `/api/field-mappings`.
  - Frontend kann jetzt zwischen Demo-Daten und Backend-Daten umschalten
    (via `API_BASE_URL` in `config.local.js`).
  - Noch offen fuer den Zielzustand: `/api/beetles/:id`, `/api/map/points`,
    `/api/countries/:countryCode` und final einheitliches Response-Format.

## Session-Update (2026-06-08) — Backend Phase A

Scope dieser Session:

- Nur Backend gearbeitet, keine Frontend-Aenderungen.
- API-Vertrag/Pflichtenheft gegen Implementierung abgeglichen.

Umgesetzt in `Database/backend/main.py`:

- Einheitliches Fehlerformat eingefuehrt:
  - `{"error": "...", "message": "..."}`
- `GET /api/beetles` liefert jetzt paginierte Listenstruktur:
  - `items`, `total`, `page`, `page_size`
- `GET /api/beetles/{beetle_id}` hinzugefuegt:
  - akzeptiert `occ-<id>` und `<id>`
- `GET /api/countries/{country_code}` hinzugefuegt:
  - liefert `code`, `name`, `speciesCount`, `topClimates`, `topVegetations`, `elevationRange`

Filterstatus:

- Das erweiterte Filterset auf `/api/beetles` bleibt gueltig und soll genutzt werden.
- `/api/filters` bleibt absichtlich breit und liefert neben den Kernfeldern auch
  Band-/Klassenwerte fuer die erweiterten Backend-Filter.

Doku-Updates dieser Session:

- `docs/API-VERTRAG.md` an Implementierungsstand angepasst
  - paginierte Antwort fuer `/api/beetles`
  - englisch codierte Klassenwerte dokumentiert
  - Hinweis aufgenommen, dass die erweiterten Filter gueltig sind und genutzt werden sollen
- `docs/PFLICHTENHEFT.md` aktualisiert
  - `/api/beetles/:id` und `/api/countries/:countryCode` als vorhanden markiert
  - offener Endpunkt fuer Zielzustand: `/api/map/points`

- erledigt: `/api/map/points` inkl. Bounding-Box-Filter und Zoom-basiertem Clustering
- erledigt: einheitliche Pagination auf weiteren Listenendpunkten umgesetzt (`/species`, `/observations`)
- erledigt: `/api/filters` liefert jetzt `core|extended` (Standard: `core`, `extended` = Core + Extended)
- erledigt: `GET /api/map/points/geojson` ergaenzt
- erledigt: Performance-Feinschliff fuer Kartenabfragen gestartet
  - BBox-Filter in `list_beetles` frueher in die Base-CTE geschoben
  - SQL-Indexskript fuer Kartenabfragen angelegt: `Database/SQL/AddMapQueryIndexes.sql`

Naechste offene Backend-Schritte:

- Indexskript auf Ziel-DB ausrollen und EXPLAIN vor/nachher dokumentieren

## Backend Session (2026-06-10) — Backend Next Steps 1-5

### Sprint 1: Stabilisieren und messen (abgeschlossen)

- DB-Fehlerhandling vereinheitlicht (strukturierte API-Fehler statt unspezifischer Antworten).
- CORS-Parsing/Config zentralisiert.
- Sort-Whitelist und Pagination-Guards konsequent in Controller-Helpern umgesetzt.
- Slow-Query-Logging konfigurierbar gemacht (`ENABLE_SLOW_QUERY_LOGGING`, `SLOW_QUERY_MS`).
...
- Climate-Join fachlich korrekt auf event_date-as-of umgestellt und erneut gemessen.

### Sprint 3: Architektur aufraeumen (abgeschlossen)

- Schichtung auf Router -> Controller -> Repository umgesetzt.
...
- API-Vertragstests fuer Kernendpunkte umgesetzt unter `Database/Backend/App/tests/test_api_contract.py`.
- Testkonfiguration angelegt (`Database/Backend/App/tests/pytest.ini`, `Database/Backend/App/tests/requirements-test.txt`).
- OpenAPI-Beispiele fuer Vertragsendpunkte in Routern ergaenzt.
- Beispielpayloads zentralisiert in `Database/Backend/App/core/openapi_examples.py`.
- Testlauf validiert: `python -m pytest` in `Database/Backend/` -> `6 passed in 18.95s`.
- Quality-Endpoint umgesetzt: `GET /quality/report` mit Nullraten fuer Observation/Location/ClimateSnapshot und EE-Coverage-Quote.

### Sprint 6: Datenmodell und Medien (in Arbeit)

- Event-Date-Normalisierung gestartet (`event_date_parsed`) inkl. Schema-/Load-/Migrationsskript.
- Bildfelder im Beetle-Response erweitert (`imageUrl`, `meta.media.items`).
- Neuer dedizierter Medien-Endpoint umgesetzt: `GET /api/beetles/{id}/media` (paginiert mit `limit`/`offset`).
- API-Vertrag aktualisiert fuer:
  - paginierte Antwort von `/api/beetles`
  - Bildfelder in `/api/beetles` und `/api/beetles/{id}`
  - neuen Endpunkt `/api/beetles/{id}/media`
